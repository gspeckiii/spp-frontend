...
function AdminCategoryPutSelect() {
  ...
  if (!user.token) {
    return (
      <p className="text-danger">Please log in as admin to view categories</p>
    );
  }
      ...
      <div className="table-select">
        <div className="table-select__section-heading">
          ...
          <Link
            ...
            className="table-select__button table-select__button--success table-select__button--large"
          >
            Add Category
          </Link>
        </div>
  ...
  const uniqueCategories = Array.from(
    new Map(categories.list.map((cat) => [cat.cat_id, cat])).values()
  );
