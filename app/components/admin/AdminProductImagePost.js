...
function AdminProductImagePost() {
    ...
    async function fetchProduct() {
      try {
        ...
        if (!token) {
          appDispatch({
            type: "flashMessage",
            value: "Please log in as admin to add images",
          });
          setLoading(false);
          return;
        }
        ...
        appDispatch({
          type: "flashMessage",
          value: e.response ? e.response.data.error : "Error fetching product",
        });
        setLoading(false);
      }
    }
  ...
  const handleFileChange = (e) => {
    ...
    if (files.length === 0) {
      ...
      if (invalidFiles.length > 0) {
        setValidationError(
          "All images must be JPEG, PNG, or GIF and less than 5MB."
        );
      } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      appDispatch({
        type: "flashMessage",
        value: e.response ? e.response.data.error : "Error adding images",
      });
    }
  };
        ...
        <form onSubmit={handleSubmit}>
          <div className="form__group">
            ...
            <small style={{ color: "$softWhite", opacity: 0.7 }}>
              Select multiple images (JPEG, PNG, GIF, max 5MB each)
            </small>
            <CSSTransition
              in={!!validationError}
              timeout={330}
          ...
          <div className="form__group">
            <button
              ...
              disabled={!!validationError || images.length === 0}
            >
              Add Images
            </button>
            <button
